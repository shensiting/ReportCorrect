...
<script type="text/javascript">
	...
	function editFunc(){
		...
		if (ids.length != 1 ) {
			...
			//跳转到页面
			window.location.href='<%=basePath%>manage/addTeacherExperiment.html?id='+id;
		}
	}
		...
		$.teninedialog({
			...
			clickButton : function(sender, modal, index) {
				...
				$.ajax({
					type : 'get',
					dataType : 'json',
					url : '<%=basePath%>manage/delTeacherExperiment.action?ids='
							+ ids,
	...
	<div id="content">
		...
		<div id="breadcrumb">
			<a href="<%=basePath%>manage/index" title="主页" class="tip-bottom"><i class="icon-home"></i>
				首页</a> <a href="#" class="current">考核信息管理</a>
		</div>
				...
				<div class="span12">
					<div class="btn-group">
						<a class="btn btn-sm tip-bottom" title="新增"
							href="<%=basePath%>manage/addTeacherExperiment"><i
					...
					<div class="widget-box">
						...
						<div class="widget-content nopadding">
							...
							<table class="table table-bordered data-table  with-check ">
								...
								<tbody>
									<c:forEach var="item" items="${teacherlinkExperiments}">
										<tr>
											<td><input name="checkboxid" type="checkbox"
												value="${item.getcId()}" /></td>
												<td>${item.getTeacher().getcTeacherId()}</td>
											<td>${item.getTeacher().getcName()}</td>
											<td>${item.getExperimentalTest().getcExperimentName()}</td>
