...
<script type="text/javascript">
	...
	function editFunc(){
		...
		if (ids.length != 1 ) {
			...
			var id= ids[0];
			//跳转到页面
			window.location.href='<%=basePath%>manage/addGrade.html?id='+id;
		}
	}
				...
				<div class="span12">
					<div class="btn-group">
						<a class="btn btn-sm tip-bottom" title="新增"
							href="<%=basePath%>manage/addGrade.html"><i class="icon-plus"></i></a>
									...
									<c:forEach var="item" items="${gmclist}">
										<tr>
											<td><input name="checkboxid" type="checkbox"
												value="${ item.cId}" /></td>
											<td>${item.cYearClass}</td>
											<td>${item.major.cMajorName}</td>

											<td>${item.cClass}</td>
											<td>${item.college.cCollegeName}</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
