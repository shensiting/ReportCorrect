...
function sumit(testform) {
	$.ajax({
		...
		dataType : 'json',
		async: false,
		url : '<%=basePath%>manage/addTest.action?id=' + testform.id.value+'&testName='+testform.testName.value+'',
...
<body>
	...
	<span class="c-gray en">&gt;</span> 实验分类管理 <a
		...
		<div style="display: none;" id="addinfo" class="pd-20">
			<form class="form-horizontal" method="post" action=""
				...
				<input type="hidden" id="id" name="id" value="" /> <input
					...
					placeholder="输入实验分类" required="required" id="testName"
					name="testName"> &nbsp;&nbsp;&nbsp;
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			btn.bind("click",function(){
				...
				document.getElementById("testName").value=btn.parent().parent().children("td").get(2).innerHTML;
		...
		function deleteFunc() {
			...
			layer.confirm(
					...
					$.ajax({
						type : 'get',
						dataType : 'json',
						url : '<%=basePath%>manage/delTest.action?ids=' + ids,
						success : function(data) {
							if (data.success) {
								//删除成功，刷新页面
								alert("删除成功！");
		...
		function article_del(obj, id) {
			layer.confirm('确认要删除吗？', function(sender, modal, index) {
				...
				$.ajax({
					type : 'get',
					dataType : 'json',
					url : '<%=basePath%>manage/delTest.action?ids=' + id,
					success : function(data) {
						if (data.success) {
							alert("删除成功！");
						} else {
