...
<script type="text/javascript">
	...
	function editFunc(){
		...
		if (ids.length != 1 ) {
			...
			var id= ids[0];
			//跳转到页面
			window.location.href='<%=basePath%>manage/addStudent.html?id='+id;
		}
	}
		...
		$.teninedialog({
			...
			clickButton : function(sender, modal, index) {
				...
				$.ajax({
					type : 'get',
					dataType : 'json',
					url : '<%=basePath%>manage/delStudent.action?ids='
							+ ids,
	...
	<div id="content">
		...
		<div id="breadcrumb">
			<a href="<%=basePath%>manage/index" title="主页" class="tip-bottom"><i class="icon-home"></i>
				首页</a> <a href="#" class="current">学生信息管理</a>
		</div>
				...
				<div class="span12">
					<div class="btn-group">
						<a class="btn btn-sm tip-bottom" title="新增"
							href="<%=basePath%>manage/addStudent.html"><i
								...
								<tbody>
									<c:forEach var="item" items="${studentGrades}">
										<tr>
											...
											<td>${item.getcName()}</td>
											<td>${item.getGrade().getcYearClass()}</td>
											<td>${item.getMajor().getcMajorName()}</td>
											<td>${item.getGrade().getcClass()}</td>
											<td>${item.getCollege().getcCollegeName()}</td>
											<td>${item.getcIDNumber()}</td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
