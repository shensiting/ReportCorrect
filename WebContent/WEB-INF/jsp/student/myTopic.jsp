...
<body>
	...
	<div class="pd-20" style="margin-left: 5%; margin-right: 5%;">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<span class="l"><a class="btn btn-success radius"
				onclick="article_add('提问','<%=basePath%>student/askQuestion','1000','550')"><i
					class="Hui-iconfont">&#xe600;</i>新增提问</a> </span>
		</div>
					...
					<c:forEach var="item" items="${topicThemes}">
						<tr class="text-c">
							<td class="text-l"><a class="maincolor" href="javascript:;"
								onclick="qusShow('${item.getcId()}')">${item.getcTitle()}</a> <span
							...
							<td>${item.getcCommentNum()}</td>
							<td><a class="maincolor"
								onClick="article_del(this,'${ item.cId}')" href="javascript:;">删除</a></td>
						</tr>
					</c:forEach>
	...
	<script type="text/javascript">
		...
		function qusShow(id) {
			var url='<%=basePath%>student/showMyTopic?id='+id;
