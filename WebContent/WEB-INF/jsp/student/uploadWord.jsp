               ...
               //返回true，代表不允许上传此实验
            	if(data.mes!=""){
            		...
            		//此处作用是当用户连续选择实验错误后，会导致selected一直处于在第一个被选中，解决出错问题使用
            		 $("#experiment").find("option").eq(0).attr("selected",true);
            		 $("#experiment").find("option").eq(0).attr("selected",false);
            	}
...
function load(){         
    	...
    	if(experiment==""){
    		...
    		document.forms[0].action='<%=basePath%>student/uploadWord.action';
				...
				<div class="formControls col-5">
					<span class="select-box"> <select id="experiment"
						name="experiment" class="select">
							...
							<c:forEach var="item" items="${experiment}">
